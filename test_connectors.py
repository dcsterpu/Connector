...
class ConnectorDescriptor(unittest.TestCase):
    ...
    def test_TRS_CONNECTOR_TEST(self):
        current_path = os.path.realpath(__file__)
        head, tail = ntpath.split(current_path)
        os.system('ConnectorDescriptor.py -config ' + head + '\\tests\TRS.CONNECTOR.TEST\\ConfigConnectorDescriptor.xml')
        self.assertTrue(FileCompare.areSame(head + '\\tests\TRS.CONNECTOR.TEST\output\Connectors.arxml', head + '\\tests\TRS.CONNECTOR.TEST\Connectors.arxml'))
